<template>
  <div class="q-pa-md q-gutter-sm" style="padding-left: 20px;padding-top: 20px;padding-bottom: 0">
    <q-breadcrumbs>
      ...
      <q-breadcrumbs-el label="歌单管理" icon="fa-solid fa-guitar"/>
    </q-breadcrumbs>
  </div>
  <div class="q-pa-md">
    <q-table
        v-model:pagination="pagination"
        v-model:selected="selectedPlaylists"
        :columns="columns"
        :filter="filter"
        :loading="loading"
        :rows="rows"
    ...
    >
      ...
      <template v-slot:body-cell-status="props">
        <q-td :props="props">
          <div>
            <q-btn
                :label="props.row.status==='PUBLISH'?'已发布':(props.row.status==='CLOSE'?'已关闭':'草稿')"
                :color="props.row.status==='PUBLISH'?'primary':(props.row.status==='CLOSE'?'deep-orange':'blue')"
                outline
      ...
      <template v-slot:body-cell-special="props">
        <q-td :props="props">
          <div>
            <q-btn
                :label="props.row.special=== false ?'否':'是'"
                :color="props.row.special=== false ?'primary':'red'"
                outline
                @click="reverseSpecial(props.row.id,props.row.special)"
                style="font-weight: bolder;font-size: 10px"/>
          </div>
        </q-td>
      </template>
      ...
      <template v-slot:body-cell-operation="props">
        <q-td :props="props">
          <q-btn color="primary" icon="edit" @click="editPlaylist(props.key)" style="margin-right: 10px"/>
          <q-btn color="dark" icon="delete" @click="removePlaylist([props.key])"/>
        </q-td>
      </template>
      <template v-slot:top>
        <q-btn :disable="loading" color="primary" label="创建歌单" @click="addPlaylist"/>
        <q-btn :disable="loading" class="q-ml-sm" color="primary" label="删除歌单" @click="removePlaylist"/>
        <q-space/>
        <q-select
...
import {ref} from "vue";
import {playListManage} from "../../compose/playlistManage.js";
...
export default {
  ...
  setup() {
    const columns = [
      ...
      {name: 'createdUser', align: 'center', label: '创建者', field: 'createdUser', sortable: true},
      {name: 'updatedUser', align: 'center', label: '更新者', field: 'updatedUser', sortable: true},
      {name: 'special', align: 'center', label: '特色歌单', field: 'special'},
    ...
    return {
      columns,
      visibleColumns: ref(['cover', 'status', 'createdUser', 'special', 'recommended', 'recommendFactor', 'operation']),
      ...playListManage()
    }
  }
}
