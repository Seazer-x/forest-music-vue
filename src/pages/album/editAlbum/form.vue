  ...
  <div class="row user-form">
    <q-card bordered flat style="width: 50vw;margin: 45px auto auto;">
      <q-card-section v-if="pathId">
        <div class="text-h6">修改专辑信息</div>
      </q-card-section>
      <q-card-section v-else>
        <div class="text-h6">发布专辑</div>
      </q-card-section>
      ...
      <q-card-section>
        <q-form style="width: 45vw">
          <q-input v-model="AlbumInfo.name" :loading="loading" class="input-form" label="专辑名"
                   standout="bg-teal text-white" type="text"/>
          <q-toggle v-model="AlbumInfo.recommended"
                    :label="`${AlbumInfo.recommended===false?'未推荐':'开启推荐'}`"
                    checked-icon="fa-solid fa-fire"
                    class="input-form"
                    color="red"
                    unchecked-icon="fa-regular fa-snowflake"/>
          <br>
          <q-badge v-if="AlbumInfo.recommended" color="primary"
                   style="font-weight: bold;font-size: 15px;height: 19px">
            推荐因数: {{ AlbumInfo.recommendFactor }}
          </q-badge>
          <q-item v-if="AlbumInfo.recommended">
            <q-item-section avatar>
              <q-icon color="red" name="fa-brands fa-cloudsmith"/>
            </q-item-section>
            <q-item-section>
              <q-slider
                  v-model="AlbumInfo.recommendFactor"
                  color="green"
                  markers
                  snap
                  switch-label-side
          ...
          <div class="row" style="padding-top: 10px">
                ...
                <template v-slot:header="scope">
                  <div class="row no-wrap items-center q-pa-sm q-gutter-xs">
                    ...
                    <div class="col">
                      ...
                      <div class="q-uploader__subtitle">{{ AlbumInfo.cover ? "已有" : "未上传" }}</div>
                    </div>
            ...
            <q-img v-if="AlbumInfo.cover!==''" class="rounded-borders" spinner-color="primary"
                   :src="AlbumInfo.cover"
          ...
          <q-input v-model="AlbumInfo.description" :loading="loading" class="input-form" label="简介" outlined standout
                   type="textarea"/>
          <q-toggle v-model="AlbumInfo.status"
                    :label="`${AlbumInfo.status==='BLOCKED'?'封禁':(AlbumInfo.status==='PUBLISH'?'发布':'草稿')}`"
  ...
  setup() {
    ...
    function upload(files) {
      ...
      fileUpload.uploadFile(fileData, 'cover').then(res => {
        ...
        coverOK.value = true
        AlbumInfo.value.cover = res.data.data.cover
      })
    }

    function removeFile() {
      coverOK.value = false
      AlbumInfo.value.cover = oldCover
    }
    ...
    function fetchData(pathId) {
      ...
      albumApi.getAlbumById(pathId).then(res => {
        ...
        loading.value = false
        oldCover = AlbumInfo.value.cover
      })
    }
    ...
    function onSubmit() {
      ...
      if (id) {
        ...
        albumApi.addAlbum(album).then(response => {
